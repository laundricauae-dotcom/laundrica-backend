...
import { LangCacheDefaultError } from "../models/errors/langcachedefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | LangCacheDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | LangCacheDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LangCacheDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LangCacheDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
